...
The "Stații de încărcare" card embeds a Google Map (no key needed) centered on
EV charging stations in Bucharest. It's interactive (pan/zoom). The buttons open
full Google Maps for search and turn-by-turn navigation. To recenter on a
different area, edit the `q=` part of the iframe `src` in index.html.
